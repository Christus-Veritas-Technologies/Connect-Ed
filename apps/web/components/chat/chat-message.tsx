        ...
        {showAvatar ? (
          <div
            className={`size-8 rounded-full flex items-center justify-center text-xs font-semibold ${
              isOwn ? "bg-brand/20 text-brand" : "bg-gray-200 text-gray-600"
            }`}
          >
            {initials}
          </div>
          ...
          <div className={`flex items-center gap-1.5 mb-0.5 ${isOwn ? "flex-row-reverse" : ""}`}>
            ...
            <span
              className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${
                ROLE_COLORS[message.senderRole] || "bg-gray-100 text-gray-600"
              }`}
            >
              {message.senderRole}
            </span>
...
function PrimitiveCard({ message, isOwn }: { message: ChatMessage; isOwn: boolean }) {
  ...
  return (
    <div
      className={`border rounded-xl overflow-hidden w-64 shadow-sm ${
        isOwn ? "ml-auto" : ""
      }`}
    >
      {/* Card header */}
